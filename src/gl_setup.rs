...
fn attach_mouse_down_handler(canvas: &HtmlCanvasElement) -> Result<(), JsValue> {
    let handler = move |event: web_sys::MouseEvent| {
        super::store::update_mouse_down(event.client_x() as f32, event.client_y() as f32, true);
    };
...
fn attach_mouse_up_handler(canvas: &HtmlCanvasElement) -> Result<(), JsValue> {
    let handler = move |event: web_sys::MouseEvent| {
        super::store::update_mouse_down(event.client_x() as f32, event.client_y() as f32, false);
    };
...
fn attach_mouse_move_handler(canvas: &HtmlCanvasElement) -> Result<(), JsValue> {
    let handler = move |event: web_sys::MouseEvent| {
        super::store::update_mouse_position(event.client_x() as f32, event.client_y() as f32);
    };
